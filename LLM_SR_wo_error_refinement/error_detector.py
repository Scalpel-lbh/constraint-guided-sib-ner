...
class ErrorDetector:
    ...
    def detect_errors(self, entities: List[Dict], abstract: str) -> List[Dict]:
        ...
        # Ablation: w/o error refinement.
        # Keep rule_errors internally only to suppress unreliable missing probes,
        # but do not send existing-entity errors to the LLM for correction.
        
        # --- 步骤 2: 查漏检测 (Missing Entity Detection) ---
        # 传递已经查出的错误，防止对错误的连词（如 "N-doped carbon"）执行数量一致性检查
        missing_errors = self._check_missing_entities(entities, abstract, existing_errors=rule_errors)
        # 将漏标错误放到最前面，大模型优先看漏标
    ...
    def format_feedback(self, errors: List[Dict]) -> str:
        ...
        feedback_lines.append("Keep your correct existing annotations unchanged. Follow the instructions to modify flagged entities, and explicitly APPEND any missing entities found.")
        return "\n".join(feedback_lines)
