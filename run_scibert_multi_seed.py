...
BASELINE_DIR = ROOT_DIR / "baseline_scibert"
EXPERIMENT_DIR = ROOT_DIR / "seed_experiments" / "scibert"
LOCAL_MODEL_DIR = (
    Path(os.environ["SCIBERT_LOCAL_MODEL_DIR"])
    if os.environ.get("SCIBERT_LOCAL_MODEL_DIR")
    else None
)
...
def run_seed(seed: int) -> None:
    ...
    env = os.environ.copy()
    env["SCIBERT_SEED"] = str(seed)
    env["SCIBERT_OUTPUT_DIR"] = str(output_dir)
    if LOCAL_MODEL_DIR and LOCAL_MODEL_DIR.exists():
        env["SCIBERT_MODEL_NAME"] = str(LOCAL_MODEL_DIR)
        env["TRANSFORMERS_OFFLINE"] = "1"
    env.setdefault("KMP_DUPLICATE_LIB_OK", "TRUE")
    env.setdefault("PYTHONIOENCODING", "utf-8")

    print(f"\n=== Running SciBERT seed {seed} ===")
    subprocess.run(
