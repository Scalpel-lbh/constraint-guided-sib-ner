...
BASELINE_DIR = ROOT_DIR / "baseline_matscibert"
EXPERIMENT_DIR = ROOT_DIR / "seed_experiments" / "matscibert"
...
def run_seed(seed: int) -> None:
    ...
    env = os.environ.copy()
    env["MATSCIBERT_SEED"] = str(seed)
    env["MATSCIBERT_OUTPUT_DIR"] = str(output_dir)
    env.setdefault("KMP_DUPLICATE_LIB_OK", "TRUE")
    env.setdefault("PYTHONIOENCODING", "utf-8")

    print(f"\n=== Running MatSciBERT seed {seed} ===")
    subprocess.run(
...
def summarize() -> None:
    rows = []
    for result_path in sorted(EXPERIMENT_DIR.glob("seed_*/results.json")):
